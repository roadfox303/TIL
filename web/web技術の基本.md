...
|要素|説明|
|:---:|:---|
|機密性|アクセスを認められたものだけがその情報にアクセスできる状態|
|完全性|情報が破壊、改ざん、消去されていない状態|
|可用性|必要なときはいつでも情報にアクセスできる状態|

- リスク・脅威・脆弱性
- リスクを現実化させる要因が「脅威」、脅威に対する弱みが脆弱性。
- 実際に不正アクセスを受け、損失が現実化することを「リスクが顕在化する」という。

### パスワードクラッキング、DoS攻撃
##### パスワードクラッキング
- 辞書攻撃
  - よくパスワードに使われる単語をまとめたファイルを用意しておいて、ログインを次々と試す。
- ブルートフォース攻撃
  - 文字の組み合わせを総当たり
##### Dos攻撃
- **DoS** (**D**enial **o**f **S**ervice)
- 短時間にサーバーが処理できないような大量のアクセスで攻撃する。
- **SYN Flood攻撃**。TCPのやり取りにおけるSYNパケットだけを大量に送りつけサーバーを接続待ち状態にさせることで別のユーザーからの新たな接続を阻害する。
- **F5攻撃** 繰り返しアクセスし続けてリクエストへの反応ができないレベルまでWebサーバーの負荷を高める。

### Webシステムの特徴を利用した攻撃
##### セッションハイジャック
- ネットワークの盗聴などの手段でCoockieの中身やセッションIDのを取得することで、ユーザーIDやパスワードを知らなくてもその情報を使ってログインユーザーとしてセッションを乗っ取る。
- 通信を暗号化したり、ログインしてきたユーザーが急に違うIPアドレスから接続してきた場合に強制的にログアウトさせるなどの対処。

##### ディレクトリトラバーサル
- URLのディレクトリ構造を逆手に取り、「現在の階層」「現在の一つ上の階層」などを指定し、公開されていないディレクトリにアクセスしWebサーバー自体のログインパスワードを取得することで不正アクセスする。

### Webアプリケーションの脆弱性を狙う攻撃
##### クロスサイトスクリプティング
- **C**ross **S**ite **S**cripting : **XSS**
- 掲示板のようなユーザーの入力内容を表示するWebサイトの脆弱性を突く攻撃。
##### クロスサイトリクエストフォージェリ
- **C**ross **S**ite **R**equest Forgeries : **CSRF**
##### SQLインジェクション
- フォームにSQLコマンドを混ぜ込む。

### システムの脆弱性
##### セキュリティホール
- ソフトウェアの欠陥で、権限を持たない操作をできてしまったり、見えてはいけない情報が第三者に見えてしまう不具合。
- ApatchなどメジャーなWebサーバーや、MySQLなどのDBサーバーからも日々見つかっている。
- 発見されたセキュリティホールは、**脆弱性情報データベース** というデータベースで管理され一般に公開されている。

##### ゼロデイ攻撃
発見されたセキュリティホールに対する修正パッチが開発される前に、そのセキュリティホールを利用した攻撃を仕掛けること。

### ファイアウォール
- インターネットと内部ネットワークの間に設置し、送受信されるデータを監視して通信の許可、拒否を行うもの。

##### パケットフィルタ型ファイアーウォール
- 送受信されるデータ（パケット）のIPアドレスとポート番号をチェックして通信の許可、拒否を判断する。

### IDS、IPS
- ファイアーウォールで防ぎきれない攻撃を防ぐ手段。
- **IDS** (**I**ntrusion **D**etection **S**ystem)、**IPS** (**I**ntrusion **P**ervention **S**ystem)
- どちらもネットワークの通信を監視し、不正な通信や普段と異なる異常な通信を検知する装置。
- IDSは、異常があったことを管理者に通知する。
- IPSは、通知に加えて即座に該当の通信を遮断する。

##### シグネチャ型(不正検知型)
- シグネチャとは既知の攻撃手法の通信パターンが登録されたデータベース。
- 監視対象の通信とシグネチャを比較し不正アクセスを判断する。

##### アノマリー型(異常検知型)
- 普段の通信とは大きく異なる通信や、通常は発生しない通信を不正アクセスと判断する検知方法。

### WAF
- **WAF** (**W**eb **A**pplication **F**irewall)
- IDSやIPSでは通信の中身まではチェックしないので、クロスサイティングスクリプティングやSQLインジェクションのような、一見正常な通信は検出できない。
- パケットの中身をみて悪意のあるデータが含まれていないかチェックする。

##### ブラックリスト型
- IDSのシグネチャのように、特定のパターンを遮断する。
- 遮断するデータのパターンを **ブラックリスト** という。

##### ホワイトリスト型
- 正常なデータのパターンのみ通信を通す。
- ホワイトリストは予め作成しておく。
- 正確なホワイトリストの作成には専門知識がいる。

### 暗号化
- 元のデータを **平文** 。暗号化アルゴリズムにより第三者が読み取れないデータにすることを暗号化という。
- 元の平文に戻すことを **復号** という。
- 暗号化とは少し異なるが、パスワードのように復号しなくても（人間には読めなくてもいい）
正しいかどうかだけ判断できれば良いものであれば、ハッシュ化して保存しておくことも対策としてアリ。
- ハッシュ化とは、ハッシュ関数と呼ばれる計算式により、任意の長さの文字列を固定長の文字列（ハッシュ値）に変換すること。ハッシュ値の長さはハッシュ関数の種類によって異なる。
- 同じハッシュ値を持つ別のデータを生成することは極めて困難。
- ハッシュ値からもとのデータを割り出すことも非常に困難なので万が一ハッシュ値が流失しても安心。

### 公開鍵証明書
- 認証局と呼ばれる第三者機関から発行される。
- HTTP通信の相手が誰なのかを証明する。
- 公開鍵の持ち主が実在することを証明する。
- 偽造に強く、偽造されてもそれを検知できる作りになっている。
##### 自己証明書
- 公開鍵証明書を認証局から発行してもらうには、発行費用と審査のための時間がかかる。
- 身内で試験的に公開鍵証明書を利用する場合に自らを認証局とした公開鍵証明書を作成できる。
- 通称 **「オレオレ証明書」**

### 認証
- 予め与えられたIDを使ってログインし本人確認をする。このような本人確認処理を認証という。
- ユーザーは多数のサービスに登録する時代になり、ユーザーとしては自信が管理するアカウントの数の増加、サイト運営者としては個人情報管理の負担が大きくなった。
- 大きなWebサービスが管理しているアカウント情報を利用して認証を行う技術が開発された。

##### 認証API
- 認証処理を提供する側（A）は、処理をAPIとして（B）に提供、（B）はユーザーをAPIに誘導し認証結果の通知をもらう。
- ユーザーは(A)にアカウントを持っている必要がある。
- 各社でAPIの規格が統一されていないのでWebアプリケーションを複数の認証APIに対応させるのに手間がかかる。

##### OpenID
- 認証APIの問題を解決するため、認証処理を標準化したプロトコル。

### 認可
- 認証結果により、ユーザー毎に権限に沿った利用を許可することを **「認可」** という。

##### OAuth
- サイトを跨いだ認可を実現するために標準化されたプロトコル。
- 機能としては認可のみで、認証は行わないため基本的には認証を行う他のプロトコルと併用される。
- 利用したいサービスを **リソース** とよぶ。サービスを提供するサーバーをリソースサーバー、そのオーナーがリソースオーナー、認可を受けてリソースを使用する側のWebサービスをクライアントとよぶ。
- クライアントがリソースを利用するときは、リソースオーナーに許可を要求し、許可が得られれば、その報告とリソースを使用するためのトークン（合言葉）の発行を依頼する。クライアントはトークンを用いることでリソースを使用できる。

##### OpenID Connect
- **OAuth2.0** をもとに認証機能が追加されたプロトコル。

### CAPTCHA
- プログラミング技術の発展で、文字入力やクリックの動作をプログラムに行わせることも可能となっている。
- そのようなプログラムの悪用を防ぐ目的で開発された。
- コンピュータと人間を識別する仕組み。
- 歪んだ文字の画像とか信号機を選ぶものとかのアレ。
...
- ミドルウェア
  ...
  - Webサーバー、APサーバー、DBMSなどがこれにあたる。

### ネットワーク構成の検討
- Webシステムの用途や要求されるセキュリティレベル、投入できるコストなどによって構成は変わる。
##### ネットワークの分割
- 外部ネットワーク（インターネットに晒される）
- 内部ネットワーク(インターネットと隔離されている)
- **DMZ**（内部、外部双方のネットワークからアクセスを受けつつ、不正な通信を防ぐため内部ネットワークへの通信は最低限）
- 分割し、機密性の高いDBサーバーなどを内部にネットワークに配置し、インターネットから直接重要なデータにアクセスされることを防ぐ。

##### ネットワーク機器の構成
- ファイヤーウォールは必須だが、IPS、IDS、WAFは高価なので求められるセキュリティ要件によっては導入しないこともある。

### サーバー構成の検討
##### ロードバランサー
- Webシステム冗長化の際、リクエストをどのWebサーバーに割り振るかをコントロールする。

##### 配置先のネットワーク
- サーバー機器はDMZもしくは内部ネットワークに配置する。
- 基本的にインテーネットとの通信が発生しないサーバー機器は内部ネットワークに配置する。
- インターネットとの通信が必要なもののみDMZに配置する。

##### サーバーの構成
- 構成するシステムの要件によって、コストを重視するのか冗長化で可用性を重視するのかなど。

### サーバー基盤の検討
##### オンプレミス
- 「自社運用」を指す。自分で機器を購入して利用する方法。自由度は高いが運用の手間や初期コストが高価になる。

##### レンタルサーバー
- 他者が構築し貸し出しているサーバーを間借りする。
- 構築や運用の手間がかからないが、構成に自由はない。

##### クラウド
- 他社が提供する仮想サーバーを設置できる環境（クラウド）の中にサーバーを設置する。
- 自由な構成が可能。

### 負荷分散
- サーバーを複数台並列に配置しリクエストを振り分けて並行で実行することでサーバーの負荷を軽減する方法を **負荷分散** とよぶ。
- 負荷分散はロードバランサーが行う。

##### ラウンドロビン方式
- 各サーバーにリクエストを順番に割り振る方式。
- リクエストの重さにばらつきがあると、各サーバーで公平な負荷分散とはならない。

##### 動的分散方式
- サーバーの負担を監視し、負荷が少ないサーバーに優先してリクエストを振り分ける方式。

##### パーシステンス
- セッション情報を管理しやすいように、同じクライアントは同じサーバーへ転送する機能。

### サーバー設計・構築
##### ディスク構成
- バクアップ用のデータなど、重要なデータはサーバーのシステムデータとは別のディスクに保管しておく。ディスクが複数用意できない場合は **パーティショニング** で分けておく。

##### セキュリティ
- 使用しないOSの初期設定（プリデフォルト）のユーザーを削除しておくなど

##### システム管理ソフト
- 最後にサーバー設定がただ行く行われているかテスト。
- 設定ファイルが正しく記載されていること、削除ユーザーでログインできないこと、ミドルウェアの動作確認、再起動に必要なプログラムが正しく起動するかなど。

### データベース設計
##### 論理設計
- データに格納すべきデータの洗い出しとデータ同士の関連。
- 必要なテーブルと、リレーションなどの設計。

##### 物理設計
- カラムの設計。
- 項目とデータ形式など。
- データベースの文字コードは何にするか、データベースに割り当てるディスク容量はどうするかなども検討。
...
##### 詳細設計
- モジュールを検討する。
- モジュールの処理内容、モジュール間の連携、画面遷移のフローなど。

##### テスト
- 単体テスト(モジュール単体の振る舞いをテスト)
- 結合テスト(喪ジュルー間の連携をテスト)

### バックアップ運用
- システムを構成するデータを保管しておく。
- バックアップデータの取得や不要な古いックアップデータの削除などの作業を、 **バックアップ運用** という。

##### バックアップの方法
- バックアップの対象は、アプリケーションや、コンテンツのようなファイルと、データベースの中身。
- バックアップは過去何世代か保管しておくことが望ましい。

### ログ運用
|種類|説明|
|:---:|:---|
|システムログ|OSが発生したイベントを記録する。|
|アプリケーションログ|ミドルウェアなどのアプリケーションが動作履歴を記録する。|
|アクセスログ|Webサーバーなどが受けたリクエストを記録する|

##### ログローテーション
- ログファイルを切り分け別のファイルとして保存する。
- 1日、1週間、1ヶ月など切りの良い単位で分けられることが多い。
- ログの容量が多い場合は100MB単位などで切り分けることもある。
- ログローテーションは **ハウスキープ** とも呼ばれる。

##### アクセスログ解析
- ユーザーがアクセスしてきた日時やページなどが記録されているため、解析して参考となる情報を得ること。

### Webサイトのパフォーマンス
##### パフォーマンスの指標
- リクエストからHTTPが帰ってくるまでの **応答時間**
- リクエスト送信からWebページの表示が完了するまでの **表示完了時間**
- ページ読み込みの開始から完了までの **ページ読み込み時間**
- エラーなくWebサイトにアクセスできた確率である **可用性**

##### パフォーマンスの監視
- 定期的にパフォーマンスを監視し、低下を検知したら原因を調査。対処する。
- CPU使用率やメモリ利用率などの **サーバー負荷状態** を監視しておく
- Zabbix、Pandra FMS、Nagios　などのツールがある。

### 脆弱性診断
- **脆弱性対策** が非常に重要
- **脆弱性データベース** を定期的に確認し、利用している製品の全弱性の有無を把握しておく。
- 実際にサイバー攻撃を行う脆弱性テスト、ペネトレーションテスト。
- 脆弱性が発見された場合、OSやミドルウェアであれば、修正パッチを適用したり脆弱性のないバージョンにアップグレードする。
- パッチやアップグレードによってWebアプリケーションが稼働する環境が変わるため十分検討が必要。
